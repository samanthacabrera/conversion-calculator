        ...
        <div className="flex flex-col items-center group space-y-6 py-4">
            ...
            <div className="mt-8 grid grid-cols-1 md:grid-cols-3 gap-6 w-full max-w-4xl px-4">
                {['Basic Math', 'Algebra', 'Geometry'].map((concept, index) => (
                    <div 
                        key={index} 
                        className={`group relative w-full h-[200px] p-4 border border-black text-black transition-all duration-300 hover:bg-black hover:text-white hover:shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] ${backgroundColors[index % backgroundColors.length]}`}
                    >
